...
\begin{enumerate}[label=(\alph*)]
  ...
  \item 显式管理版本的实测为 \textbf{31.2 ms}；unified memory 的数值应以同一机器、同一计时窗口下的实测为准。两版都必须把输入送到 GPU、并让输出可被 CPU 读取；区别在于显式版由两次 H2D 和一次 D2H copy 明确安排，而 unified memory 通常按页在 GPU 首次访问 \texttt{a}/\texttt{b}、CPU 首次扫描 \texttt{c} 时迁移。页错误处理、迁移粒度和访问位置可能使 UM 更慢；不同互连、硬件支持或运行时策略也可能令差距缩小。UM 减少的是代码中的显式搬运，不保证减少数据移动本身。
\end{enumerate}
\end{answer}
