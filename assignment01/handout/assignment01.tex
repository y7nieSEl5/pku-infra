...
\newtcolorbox{answer}{enhanced, breakable,
  colback=green!3, colframe=green!45!black, boxrule=0.5pt, arc=1.5pt,
  left=7pt, right=7pt, top=5pt, bottom=5pt,
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item \textbf{错。}TFLOPS 是单位时间内可完成的总浮点运算量（吞吐），不能推出一条指令的完成延迟；GPU 往往以更多并发线程隐藏较长的延迟。
  \item \textbf{对。}标称 HBM 带宽描述的是有足够并发、访问模式合适时的总传输吞吐；零散随机访问会受事务粒度、合并效率和访存延迟限制。
  \item \textbf{对。}严格依赖使并行度接近一，运行时间主要由关键路径上的每步延迟决定，额外的 GPU 吞吐资源几乎无从利用。
  \item \textbf{错。}1000 TFLOPS 表示理想条件下每秒约可完成 $10^{15}$ 次浮点运算，不是“一次运算花 $10^{-15}$ 秒”。
\end{enumerate}
\end{answer}
...
\begin{answer}
GPU 的毫秒级估算隐含了有大量\emph{彼此独立}的 FLOP 可以同时发射，从而把总工作量除以高吞吐。严格在线算法的第 $i+1$ 步必须等待第 $i$ 步的结果，关键路径因而接近 $10^{12}$ 步：同一时刻通常只暴露出极少量可并行工作。此时每步的算术、数据相关和访存\emph{延迟}要逐步累加，不能由更多 SM 或更高 TFLOPS 抵消。即便乐观地按每步 $1\,\mathrm{ns}$ 估算，$10^{12}$ 个依赖步骤也要 $10^3$ 秒；实际还可能有访存和控制开销。
\end{answer}
...
\midrule
thread & kernel 的最小执行单位 & 计算单元上的一个 lane & 自己的寄存器 & （自身天然有序） \\
warp & 同一 block 中通常 32 个一起调度的线程 & 一个 SM 上的 warp 与其调度器/执行 lanes & 各 lane 的寄存器 & \texttt{\_\_syncwarp()}；shuffle 等 warp 原语 \\
block / CTA & 可协作的一组线程；一个 block 驻留在一个 SM & SM 上被分配资源并驻留的 CTA & 各线程寄存器、该 block 的 shared memory & \texttt{\_\_syncthreads()}；经 shared memory 通信 \\
grid & 一次 kernel 启动的全部 blocks & 设备上所有参与执行的 SM & global memory & 普通 kernel 内无全 grid 同步；以 kernel 边界（或受限的 cooperative-groups）同步 \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
\textbf{SIMD} 是向量指令模型：一条指令显式作用于多个数据 lane，通常由程序/编译器组织向量寄存器和控制流。\textbf{SIMT} 则让程序员写许多标量 thread；硬件把它们组成 warp，并通常以同一条指令、不同线程数据和 active mask 执行。因此 SIMT 保留了每线程的寄存器、线程索引与编程抽象，而分支不同的线程会让 warp 分路径执行。

后一说法\textbf{错。}Volta 的 Independent Thread Scheduling 让线程具备更独立的执行状态，且改变了某些同步正确性要求；但分歧的路径仍要占用发射和执行资源，warp 的有效 active lanes 变少，通常仍会降低吞吐、产生性能代价。
\end{answer}
...
\midrule
CPU 单线程 & 4.696 & 1.12 \\
GPU \texttt{<<<1,1>>>} & 168.391 & 40.15 \\
GPU \texttt{<<<1,256>>>} & 2.842 & 0.68 \\
GPU 铺满 grid & 0.021 & 0.01 (16384 blocks x 256 threads) \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item \texttt{\_\_global\_\_}：这是 host 启动、device 执行的 kernel 入口。
  \item \texttt{\_\_device\_\_}：函数只在 device 上执行，且只能由 device 代码调用。
  \item \texttt{\_\_host\_\_ \_\_device\_\_}：让同一份函数定义同时生成 host 与 device 两个版本。
  \item \texttt{\_\_constant\_\_}：例如 \texttt{\_\_constant\_\_ float coeff[N];}；数据在一次 kernel 运行中只读且不变。
  \item \texttt{\_\_shared\_\_}：例如 \texttt{\_\_shared\_\_ float tile[N];}；其存储期和可见范围都限于当前 block。
\end{enumerate}
\end{answer}
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item kernel launch 对 host 是异步的。若 CPU 在 GPU 仍写 \texttt{c} 时就直接读取 unified memory，既可能读到未完成的结果，也没有建立正确的 host--device 访问顺序。因此在 CPU 读 \texttt{c} 前必须等待。这里的 \texttt{CUDA\_CHECK\_KERNEL()} 展开后调用 \texttt{cudaDeviceSynchronize()}，它就是这次显式同步。原显式管理版本也在同一个宏中同步；即便没有该宏，后面的同步 \texttt{cudaMemcpy(..., cudaMemcpyDeviceToHost)} 也会等待先前的 kernel 完成后再返回。
  \item 此机器上，显式管理为 \textbf{31.2 ms}，unified memory 为 \textbf{32.9 ms}，即 UM 慢 $1.7\,\mathrm{ms}$（约 $5.4\%$）。两版都必须把输入送到 GPU、并让输出可被 CPU 读取；区别在于显式版由两次 H2D 和一次 D2H copy 明确安排，而 unified memory 通常按页在 GPU 首次访问 \texttt{a}/\texttt{b}、CPU 首次扫描 \texttt{c} 时迁移。页错误处理、迁移粒度和访问位置可解释这次小幅变慢；不同互连、硬件支持或运行时策略也可能令差距缩小或反转。UM 减少的是代码中的显式搬运，不保证减少数据移动本身。
\end{enumerate}
\end{answer}
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item \textbf{错。}kernel launch 对 host 异步；host 会继续执行，除非通过同步 API 等待完成。
  \item \textbf{对。}同一 stream 中的操作有先后依赖；同步的 device-to-host \texttt{cudaMemcpy} 在返回给 host 前会等待此前 kernel 完成，并完成自身拷贝。
  \item \textbf{错。}启动语句通常只会立刻暴露 launch 参数等错误；kernel 执行中的非法访存通常在后续同步（如 \texttt{cudaDeviceSynchronize()}）或其他会观察到异步错误的 CUDA API 处报告。
\end{enumerate}
\end{answer}
...
\begin{answer}
grid-stride loop 令每个 thread 处理 \texttt{idx, idx + gridDim.x * blockDim.x, ...}，因而同一个 kernel 可覆盖任意 $n$，不必让 grid 大小与输入规模严格绑定。它也便于限制 launch 数量、复用 kernel，并可用 \texttt{<<<1, 1>>>} 做顺序调试。

本题的 16384 个 thread 对 $2^{24}$ 个元素而言平均每个 thread 处理 1024 个元素。正确性不受影响，且同一轮迭代的相邻 lane 仍访问相邻元素；但若 GPU 本可同时驻留更多 block，这个小 grid 会减少并发 warp 数与未完成访存数，降低隐藏内存延迟、榨取带宽的能力。循环控制也有少量额外开销。
\end{answer}
...
\begin{answer}
三次输出都包含 0--15，但例如 block 10--15 与 4--7 的相对先后发生了变化。顺序由 GPU runtime/hardware scheduler 根据可用 SM、资源占用和当时执行状态决定；device-side \texttt{printf} 的缓冲与输出也不构成顺序保证。

程序正确性\textbf{不能}依赖不同 block 的执行顺序，更不能让一个 block 等待另一个 block 的中间结果：另一个 block 可能尚未被调度。正是这条独立性约束使 grid 可以远大于 SM 数量，且同一程序可在不同规模的 GPU 上让 block 任意次序、并行或串行地执行，这就是 scalable programming model 的基础。
\end{answer}
...
\begin{answer}
完整实现见 \texttt{cuda/m2\_first\_kernel/saxpy.cu}。程序以每个 thread 处理一个元素的方式实现 \texttt{y[i] = 2.0f * x[i] + y[i]}，使用 $(n - 1) / 256 + 1$ 个 block 覆盖非零长度的输入。它在 kernel 前后记录 CUDA event，并用 \texttt{cudaEventSynchronize(stop)} 等待 event 完成后取得 kernel 时间；随后 D2H copy 将结果放回 host，double 累加得到用于判测的 \texttt{SUM}。

错误检查分两层：每个 runtime API 包在自写的 \texttt{CUDA\_CHECK} 中；kernel launch 后立即检查 \texttt{cudaGetLastError()}，event 同步还会使执行期错误可见。$n=0$ 则在 launch 之前直接输出 \texttt{SUM=0}，避免非法的零 block launch。
\end{answer}
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item 线性编号为 $3 + 8 \cdot 5 = 43$。因此它属于 warp $\lfloor 43 / 32 \rfloor = 1$，lane 为 $43 \bmod 32 = 11$。
  \item $8 \cdot 8 = 64$ 个 thread，恰好占 2 个 warp。
  \item 33 个 thread 占 2 个 warp；第二个 warp 仅有 1 个 active lane，另外 31 个 lane 在整个 kernel 中都闲置，造成执行与资源利用浪费。
\end{enumerate}
\end{answer}
...
\begin{answer}
实测中，warp 内分支为 \textbf{0.376 ms}，按 warp 分支为 \textbf{0.193 ms}，比值 \textbf{1.95}，接近 2。前者的每个 warp 都有 16 个 lane 走两个分支之一；warp 必须以 active mask 先后执行两条路径，因此每个 warp 近似承担两份循环工作。后者每个 warp 只走一条路径，32 个 lane 都活跃，两个分支的 warp 分别调度，避免了 warp 内路径串行化。

若两条路径一重一轻，奇偶分支的每个 warp 仍要串行执行\emph{重路径 + 轻路径}，时间近似由二者之和决定。warp 对齐分支则让每个 warp 只执行其所属路径；总完成时间取决于重/轻 warp 的数量、它们在各 SM 上的分布与重路径时长，重路径不再被强加给原本应走轻路径的 lane。
\end{answer}
...
\begin{answer}
\begin{enumerate}[label=(\alph*)]
  \item 每个 thread 先写 \texttt{buf[t]}，却由另一个 thread 读取 \texttt{buf[255-t]}。没有 \texttt{\_\_syncthreads()} 时，某个 warp 可以在其他 warp 完成写入前开始读 shared memory，形成跨 warp 的读写竞争；因此会读到旧值或尚未写入的值。你观测到 index 0 的 mismatch 正是这种情况。
  \item 对 256-thread block，$t$ 所在 warp 与 $255-t$ 所在 warp 始终不同：warp $w$ 的 source 来自 warp $7-w$，不存在 ``同 warp 因 lockstep 而天然有序'' 的特例。因此任何看似总是正确的位置都只是当前 GPU 的 warp 调度、旧 shared-memory 内容或时序碰巧稳定，\textbf{没有} CUDA 正确性保证。
\end{enumerate}
\end{answer}
